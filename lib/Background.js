...
export default class Background {
    constructor() {
        ...
        for (let i = 0; i < 5; i++) {
            this.addColor((i + Date.now()) % 160 + 200);
        }

        while (this.getColorsSize() < this.canvas.width * this.canvas.height) {
            this.addColor((this.colors.length + Date.now()) % 160 + 200);
        }
    }

    getColorsSize() {
        const ballSize = Math.max(this.canvas.width, this.canvas.height);
        return this.colors.reduce((acc, color) => acc + Math.PI * Math.pow(color.size * ballSize, 2), 0);
    }
    ...
    render() {
        ...
        for (let i = 0; i < this.colors.length; i++) {
            ...
            color.hue = (Math.sin(performance.now() / 5000 + i * .1) * .5 + .5) * 160 + 200;
            ...
            this.ctx.beginPath();
            this.ctx.arc(color.x + .5 | 0, color.y + .5 | 0, color.actualSize + .5 | 0, 0, Math.PI * 2);
            this.ctx.fill();
        }
